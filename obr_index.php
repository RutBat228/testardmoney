...
if (isset($montaj)) { // Предполагается, что $montaj - это массив с данными
    foreach ($montaj as $item) {
        echo '<div class="montaj-item" id="montaj_'.$item['id'].'">
                <span>'.$item['name'].'</span>
...
// Добавляем скрипт для обработки удаления
echo '<script>
    document.querySelectorAll(".delete-btn").forEach(button => {
        button.addEventListener("click", function() {
            var id = this.getAttribute("data-id");
            if (confirm("Вы действительно хотите удалить этот монтаж?")) {
                fetch("delete_montaj.php?id=" + id)
                    .then(response => response.json())
                    .then(data => {
                        if(data.success) {
                            document.getElementById("montaj_" + id).remove();
                            alert("Монтаж успешно удален!");
                        } else {
