...
if(h(isset($_GET['adress'])) && h(isset($_GET['region']))){
    ...
    $_SESSION['step'] = isset($_GET['step']) ? h($_GET['step']) : '1';
}
...
if(h(isset($_GET['oboryda'])) && h(isset($_GET['pon']))){
    ...
    $_SESSION['step'] = isset($_GET['step']) ? h($_GET['step']) : '2';
}
...
if(isset($_GET['step']) && $_GET['step'] == "final"){
    ...
    if ($results->num_rows == 1) {
        echo '<div class="alert alert-danger" role="alert">Адрес уже есть в базе</div>';
        redirect("/navigard/result.php?adress=$adress");
        exit;
    }
    ...
    VALUES ('$adress','$vihod1','$vihod2','$vihod3','$vihod4','$vihod5','$oboryda','$dopzamok','$kluch','$pred','$phone','$krisha','$lesnica','$pon','$podjezd','$log','$user','$region','$news')";

    if ($connect->query($sql) === true) {
        redirect("/navigard/result?adress=$adress");
    } else {
...
if(empty($_SESSION['adress']) && empty($_SESSION['region'])){
    ...
    if ($usr['admin'] == '1') {
        ...
        $reg = $connect->query("SELECT * FROM navigard_region ");
        echo '<small  class="form-text text-muted">В каком регионе: <b>' . $usr['region'] . '</b></small><br>';
...
if(empty($_SESSION['oboryda']) || empty($_SESSION['pon']) || $_SESSION['step'] == 1){
    ...
    $odor = $connect->query("SELECT * FROM navigard_oboryda ");
    ...
    $pon1 = $connect->query("SELECT * FROM navigard_pon ");
...
if($_SESSION['oboryda'] == 'Фасад' && $_SESSION['step'] == 2 ){
    ...
    $pod = $connect->query("SELECT * FROM navigard_podjezd ");
...
if($_SESSION['oboryda'] == 'Подвал' && $_SESSION['step'] == 2){
    ...
    $dop = $connect->query("SELECT * FROM navigard_dopzamok ");
    ...
    echo'<small  class="form-text text-muted">В каком подъезде выход на чердак</small>';
...
if($_SESSION['oboryda'] == 'Чердак' && $_SESSION['step'] == 2){
    ...
    $kri = $connect->query("SELECT * FROM navigard_krisha ");
    ...
    $les = $connect->query("SELECT * FROM navigard_lesnica ");
