    ...
    <li class="list-group-item">
        ...
        if($usr['admin'] == 1){
            ...
            if(!empty($_GET['edit'])){
                ...
                $id_user = $_GET['edit'];
                $sql = "SELECT * FROM navigard_user WHERE id = $id_user";
                    ...
                    <tr>
                        <td>
                            <small class="text-danger form-text">Смена пароля производится персонально, через администратора приложения:
                                <b>Ершова Н.Н.</b><br>Пароль нельзя <b>узнать</b>, можно только <b>сменить</b> на новый!</small><br>
                            ...
                            <small class="text-danger form-text">Регион</small>
                            <select name="region" class="form-select mr-sm-2">
                                <?php
                                $reg = $connect->query("SELECT * FROM navigard_region");
                                while ($region = $reg->fetch_object()) {
                                    $sel_region = ($region_user == $region->name) ? "selected" : "";
            ...
            echo '<div class="input-group" style="position: relative; display: flex; flex-wrap: wrap; align-items: stretch; width: 100%; justify-content: space-evenly; flex-direction: column; align-content: stretch;">';
            echo '<table>';
            
            $sql = "SELECT * FROM navigard_user ORDER BY id";
            $res_data = mysqli_query($connect, $sql);
            ...
            while($row = mysqli_fetch_array($res_data)){
                ...
                $id = $row['id'];
                ?>
                <tr style="border-color: grey; border-style: dotted; border-width: 1px;">
                    ...
                    <td>
                        <a href="?edit=<?=$id?>">
                            <i class="edite_button fa-solid fa-user-pen"></i>
                        </a>
                        <a href="JavaScript:startdel(<?=$id?>)">
                            <i style="padding: 0 10px 0;" class="delete_button fa-regular fa-trash-can"></i>
                        </a>
                    </td>
                </tr>
                <script type="text/javascript">
